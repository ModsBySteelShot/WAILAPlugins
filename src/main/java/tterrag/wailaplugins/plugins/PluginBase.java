...
public abstract class PluginBase implements IPlugin {
    ...
    protected void addConfig(String key, boolean def) {
        ConfigHandler.instance().addConfig(
                PluginRegistrar.getPluginName(this.getClass()),
                getKey(key),
                configLang.localize(String.format("config.%s.%s", PluginRegistrar.getPluginName(getClass()), key)),
                def);
    }
    ...
    @Override
    public final List<String> getWailaHead(ItemStack itemStack, List<String> currenttip, IWailaDataAccessor accessor,
    ...
    @Override
    public final List<String> getWailaBody(ItemStack itemStack, List<String> currenttip, IWailaDataAccessor accessor,
            IWailaConfigHandler config) {
        if (enabled()) {
    ...
    @Override
    public final List<String> getWailaTail(ItemStack itemStack, List<String> currenttip, IWailaDataAccessor accessor,
            IWailaConfigHandler config) {
        if (enabled()) {
    ...
    @Override
    public final NBTTagCompound getNBTData(EntityPlayerMP player, TileEntity te, NBTTagCompound tag, World world, int x,
            int y, int z) {
        if (enabled()) {
