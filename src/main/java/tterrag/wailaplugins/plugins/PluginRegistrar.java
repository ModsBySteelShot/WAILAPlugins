    ...
    public void preInit(FMLPreInitializationEvent event) {
        ...
        for (ASMData data : datas) {
            ...
            if (allModsLoaded(deps)) {
                if (name == null) {
                    if (deps.isEmpty()) {
                        throw new IllegalArgumentException(
                                "Name and deps cannot both be null! Culprit: " + data.getClassName());
                    }
                    ...
                    try {
                        ...
                        if (IPlugin.class.isAssignableFrom(clazz)) {
                            try {
                                ...
                                WailaPlugins.logger
                                        .error("Construtor for class " + clazz.getName() + " could not be accessed.");
                                failed = true;
                            } catch (InstantiationException e) {
                                WailaPlugins.logger
                                        .error("Class " + clazz.getName() + " does not have a default constructor");
                                failed = true;
                            }
                        } else {
                            WailaPlugins.logger.error(
                                    "Class " + clazz.getName()
                                            + " does not implement IPlugin and could not be loaded.");
                        }
                ...
                WailaPlugins.logger
                        .info("Skipping over plugin {} as its dependencies {} were not found.", name, unfound);
            }
        }
